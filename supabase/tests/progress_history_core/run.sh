#!/usr/bin/env bash
# Sprint 8.1A1-V1 — reproducible validation runner for the Progress History
# core schema. NOT A MIGRATION and never executed against a shared database.
#
# Creates a throwaway PostgreSQL cluster in a temporary directory, listening
# on a private unix socket only, applies all project migrations in filename
# order, runs the regression suite and destroys the cluster.
#
# Usage:   supabase/tests/progress_history_core/run.sh [output-log]
# Exit:    0 only when every case matched its expected SQLSTATE.
...
if [ "$(id -u)" = "0" ] && [ "${PH_TEST_REEXEC:-0}" != "1" ]; then
  ...
  REEXEC_LOG="${1:-/tmp/progress-history-core-run.log}"
  rm -f "$REEXEC_LOG"
...
LOG="${1:-/tmp/progress-history-core-run.log}"

WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/ph-core-XXXXXX")"
PGDATA="$WORKDIR/pgdata"
PGSOCK="$WORKDIR/sock"
DBNAME="progress_history_core_test"
...
# Never inherit an ambient connection: this suite must not touch a shared
