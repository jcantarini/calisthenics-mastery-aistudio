-- Sprint 8.1A3 — disposable-environment bootstrap for the outbox suite.
-- NOT A MIGRATION. Never applied to preview/production.
--
-- Provides the minimal Supabase-like surface the 20 project migrations need:
--   * roles: anon, authenticated, service_role
--   * schema auth with auth.users and auth.uid()
-- and the harness used by the outbox regression suite. It is an
-- independent copy of the core bootstrap so the core suite stays untouched.
...
-- ---------------------------------------------------------------------
-- Sprint 8.1A3-C1 — permissive default table privileges (regression env)
-- ---------------------------------------------------------------------
-- The defect fixed by the corrective migration is only observable when the
-- database hands out privileges by default: GRANT adds privileges, it never
-- removes the ones a default grant already produced. The original bootstrap
-- created a minimal-permission cluster, so it could not detect it.
--
-- These defaults are attached to the role that actually creates the migration
-- objects here (the bootstrap/migration administrator `pgtest`), never to
-- service_role, and they belong to test tooling only: no project migration
-- changes schema-wide default privileges.
