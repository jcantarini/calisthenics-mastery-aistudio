#!/usr/bin/env bash
# Sprint 8.1A3 — reproducible validation runner for the durable dispatch outbox of Progress
# History. NOT A MIGRATION and never executed against a shared database.
#
...
# runs the outbox regression suite and destroys the cluster.
#
# Usage:   supabase/tests/progress_history_outbox/run.sh [output-log]
...
if [ "$(id -u)" = "0" ] && [ "${PH_TEST_REEXEC:-0}" != "1" ]; then
  ...
  REEXEC_LOG="${1:-/tmp/progress-history-outbox-run.log}"
  rm -f "$REEXEC_LOG"
  REEXEC_CMD="PH_TEST_REEXEC=1 PATH=$PATH PH_MIGRATIONS_DIR=${PH_MIGRATIONS_DIR:-} bash $(printf '%q' "${BASH_SOURCE[0]}") $(printf '%q' "$REEXEC_LOG")"
...
MIGRATIONS_DIR="${PH_MIGRATIONS_DIR:-$REPO_ROOT/supabase/migrations}"
LOG="${1:-/tmp/progress-history-outbox-run.log}"

WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/ph-outbox-XXXXXX")"
PGDATA="$WORKDIR/pgdata"
PGSOCK="$WORKDIR/sock"
DBNAME="progress_history_outbox_test"
