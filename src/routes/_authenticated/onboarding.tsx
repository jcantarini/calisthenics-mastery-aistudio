...
function OnboardingPage() {
    ...
    (async () => {
      ...
      if (remote) {
        setData((prev) => ({
          ...remote,
          ...prev,
          equipment: prev.equipment.length ? prev.equipment : remote.equipment,
          target_areas: prev.target_areas.length ? prev.target_areas : remote.target_areas,
        }));
      }
  ...
  const toggleInArray = (k: "equipment" | "target_areas", value: string) =>
    setData((prev) => {
      ...
      return {
        ...prev,
        [k]: list.includes(value) ? list.filter((x) => x !== value) : [...list, value],
      };
    });
    ...
    switch (s) {
      case 2:
        ...
        if (!data.height_cm || data.height_cm < 100 || data.height_cm > 250)
          return tOb(locale, "ob.err.height");
        if (!data.weight_kg || data.weight_kg < 30 || data.weight_kg > 300)
          return tOb(locale, "ob.err.weight");
      ...
      <section key={step} className="mt-8 animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        {step === 12 && (
          <Step12
            data={data}
            setPerf={(k, v) =>
              setData((p) => ({ ...p, current_performance: { ...p.current_performance, [k]: v } }))
            }
          />
        )}
...
function Step2({
  ...
  return (
    <div className="space-y-5">
      ...
      <TextField
        label={tOb(locale, "ob.s2.country")}
        value={data.country}
...
function Step5({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <OptionCard
          active={data.has_experience === true}
          onClick={() => set("has_experience", true)}
          label={tOb(locale, "ob.s5.yes")}
        />
        <OptionCard
          active={data.has_experience === false}
          onClick={() => set("has_experience", false)}
          label={tOb(locale, "ob.s5.no")}
        />
      </div>
    </div>
  );
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        {AREA_OPTS.map((o) => (
          <OptionCard
            key={o}
            active={data.target_areas.includes(o)}
            onClick={() => toggle(o)}
            label={tOb(locale, `ob.s8.${o}`)}
            compact
          />
        ))}
      </div>
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        {opts.map((o) => (
          <OptionCard
            key={o}
            active={data.motivation === o}
            onClick={() => set("motivation", o)}
            label={tOb(locale, `ob.s10.${o}`)}
            compact
          />
        ))}
      </div>
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        {opts.map((o) => (
          <OptionCard
            key={o}
            active={data.skill_goal === o}
            onClick={() => set("skill_goal", o)}
            label={tOb(locale, `ob.s11.${o}`)}
            compact
          />
        ))}
      </div>
...
function Step12({
  ...
  data: OnboardingData;
  setPerf: (k: keyof OnboardingData["current_performance"], v: number | undefined) => void;
}) {
  const { locale } = useT();
...
function Step13({ data, onEdit }: { data: OnboardingData; onEdit: (s: number) => void }) {
  ...
  const rows = useMemo(() => {
    ...
    const perf =
      Object.entries(data.current_performance)
    ...
    return [
      ...
      {
        step: 2,
        label: tOb(locale, "ob.field.gender"),
        value: data.gender ? tOb(locale, `ob.s2.g.${data.gender}`) : "—",
      },
      {
        step: 2,
        label: tOb(locale, "ob.field.height"),
        value: data.height_cm ? `${data.height_cm} cm` : "—",
      },
      {
        step: 2,
        label: tOb(locale, "ob.field.weight"),
        value: data.weight_kg ? `${data.weight_kg} kg` : "—",
      },
      { step: 2, label: tOb(locale, "ob.field.country"), value: data.country ?? "—" },
      {
        step: 2,
        label: tOb(locale, "ob.field.language"),
        value:
          LOCALES.find((l) => l.code === (data.language as Locale))?.label ?? data.language ?? "—",
      },
      {
        step: 3,
        label: tOb(locale, "ob.field.level"),
        value: data.fitness_level ? tOb(locale, `ob.s3.${data.fitness_level}`) : "—",
      },
      {
        step: 4,
        label: tOb(locale, "ob.field.goal"),
        value: data.primary_goal ? tOb(locale, `ob.s4.${data.primary_goal}`) : "—",
      },
      {
        step: 5,
        label: tOb(locale, "ob.field.experience"),
        value:
          data.has_experience == null
            ? "—"
            : tOb(locale, data.has_experience ? "ob.s5.yes" : "ob.s5.no"),
      },
      { step: 6, label: tOb(locale, "ob.field.equipment"), value: eq || tOb(locale, "ob.none") },
      {
        step: 7,
        label: tOb(locale, "ob.field.schedule"),
        value: data.days_per_week
          ? `${data.days_per_week} × / ${tOb(locale, "ob.s7.days").toLowerCase()}`
          : "—",
      },
      {
        step: 7,
        label: tOb(locale, "ob.field.duration"),
        value: data.workout_duration_min ? `${data.workout_duration_min} min` : "—",
      },
      { step: 8, label: tOb(locale, "ob.field.areas"), value: areas || tOb(locale, "ob.none") },
      {
        step: 9,
        label: tOb(locale, "ob.field.injuries"),
        value: data.injuries?.trim() || tOb(locale, "ob.none"),
      },
      {
        step: 10,
        label: tOb(locale, "ob.field.motivation"),
        value: data.motivation ? tOb(locale, `ob.s10.${data.motivation}`) : "—",
      },
      {
        step: 11,
        label: tOb(locale, "ob.field.skill"),
        value: data.skill_goal ? tOb(locale, `ob.s11.${data.skill_goal}`) : "—",
      },
